...
class SLatPose(SLatVisMixin, StandardDatasetBase):
    """
    structured latent dataset
    ...
    def get_instance(self, root, instance, n_view):
        data = np.load(os.path.join(root, 'latents_pose', self.latent_model, f'{instance}_{n_view}.npz'))
        coords = torch.tensor(data['coords']).int()
        feats = torch.tensor(data['feats']).float()
        if self.normalization is not None:
            feats = (feats - self.mean) / self.std
...
class ImageConditionedSLatPose(ImageConditionedMixinRotation, SLatPose):
    """
    Image conditioned structured latent dataset
    """
    pass