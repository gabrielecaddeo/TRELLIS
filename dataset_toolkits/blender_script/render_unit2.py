...
def load_object(object_path: str) -> None:
    ...
    if file_extension == "blend":
        ...
        #     if obj.type == "MESH":
        #         # Check if the object has vertex color layers
        #         if len(obj.data.vertex_colors) == 0:
        #             print(f"Warning: {obj.name} has no vertex color layers.")
        #         else:
        #             print(f"Vertex colors found for {obj.name}.")
    else:
        import_function(filepath=object_path)
    # for obj in bpy.context.scene.objects:
    #     if obj.type == "MESH":
    #         check_vertex_colors(obj)  # Check and report if the mesh has vertex colors

    #         # Assign vertex colors to material (so they are visible)
    #         assign_vertex_color_to_material(obj)
...
def main(arg):
    ...
    for i, view in enumerate(views):
        radius_u2 = 2.0 * view['radius']
        cam.location = (
            radius_u2 * np.cos(view['yaw']) * np.cos(view['pitch']),
            radius_u2 * np.sin(view['yaw']) * np.cos(view['pitch']),
            radius_u2 * np.sin(view['pitch'])
        )
        cam.data.lens = 16 / np.tan(view['fov'] / 2)
        
        if arg.save_depth:
            half_diag_u2 = 1.0 * np.sqrt(3.0)
            spec_nodes['depth_map'].inputs[1].default_value = radius_u2 - half_diag_u2
            spec_nodes['depth_map'].inputs[2].default_value = radius_u2 + half_diag_u2
        ...
        if arg.save_depth:
            metadata['depth'] = {
            'min': radius_u2 - half_diag_u2,
            'max': radius_u2 + half_diag_u2
        }
