...
def set_render_visibility(objs, hide: bool):
    for o in objs:
        o.hide_render = hide

def set_outputs_muted(outputs: dict, muted: bool):
...
def apply_saved_normalization(saved_scale: float, saved_offset):
    ...
    scene.scale = scene.scale * float(saved_scale)
    bpy.context.view_layer.update()

    off = Vector((float(saved_offset[0]), float(saved_offset[1]), float(saved_offset[2])))
    scene.matrix_world.translation += off
    bpy.context.view_layer.update()
    return float(saved_scale), off
...
def main(arg):
    ...
    saved_tf = None
    if arg.transforms_json:
        with open(arg.transforms_json, "r") as f:
            saved_tf = json.load(f)
    ...
    # normalize scene
    if saved_tf is not None and "scale" in saved_tf and "offset" in saved_tf:
        scale, offset = apply_saved_normalization(saved_tf["scale"], saved_tf["offset"])
    else:
    ...
    if saved_tf is not None:
        ...
        for i, fr in enumerate(frames):
            ...
            # # --- object-only RGB render (no hand) ---
            # set_render_visibility(hand_objs, hide=True)

            # # mute compositor outputs so we don't write masks/depth/etc again
            # set_outputs_muted(outputs, muted=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transforms_json', type=str, default=None,
                    help='Path to an existing transforms.json to reuse exact camera poses.')
