...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, required=False, default='/home/user/TRELLIS/datasets/HSSD',
    ...
    parser.add_argument('--ckpt', type=str, default='step0130000',
    ...
    try:
        with ThreadPoolExecutor(max_workers=32) as loader_executor, \
            ...
            def loader(sha256, sdf_number):
                try:
                    sdf = get_sdf(sha256, sdf_number)
                    load_queue.put((sha256, sdf, sdf_number))
                except Exception as e:
                    print(f"Error loading SDF for {sha256} #{sdf_number}: {e}", flush=True)
            loader_executor.map(lambda x: loader(*x), jobs)
            print('loaded')
            def saver(sha256, pack, sdf_number):
                save_path = os.path.join(opt.output_dir, 'ss_latents_sdf_pose', latent_name, f'{sha256}_{sdf_number}.npz')
                np.savez_compressed(save_path, **pack)
                records.append({'sha256': sha256, f'ss_latent_{latent_name}_{sdf_number}': True})
                
            for _ in tqdm(range(len(jobs)), desc="Extracting latents"):
                sha256, ss, sdf_number = load_queue.get()
                ...
                try:
                    ...
                    assert torch.isfinite(latent).all(), f"Non-finite latent {sha256} f{sdf_number:03d}"
                    pack = {'mean': latent[0].cpu().numpy()}
                    saver_executor.submit(saver, sha256, pack, sdf_number)
