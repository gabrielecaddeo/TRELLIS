...
def _voxelize(file, sha256, output_dir, res=64):
    ...
    utils3d.io.write_ply(os.path.join(output_dir, 'voxels_unit2', f'{sha256}.ply'), centers.astype(np.float32))
    return {'sha256': sha256, 'voxelized': True, 'num_voxels': len(centers)}
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(opt.output_dir, 'voxels_unit2'), exist_ok=True)
    ...
    for sha256 in copy.copy(metadata['sha256'].values):
        if os.path.exists(os.path.join(opt.output_dir, 'voxels_unit2', f'{sha256}.ply')):
            pts = utils3d.io.read_ply(os.path.join(opt.output_dir, 'voxels_unit2', f'{sha256}.ply'))[0]
