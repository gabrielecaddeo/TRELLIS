...
def _render(file_path, sha256, output_dir, num_views):
    output_folder = os.path.join(output_dir, 'renders_unit2', sha256)
    ...
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render_unit2.py'),
        '--',
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(opt.output_dir, 'renders_unit2'), exist_ok=True)
    ...
    for sha256 in copy.copy(metadata['sha256'].values):
        if os.path.exists(os.path.join(opt.output_dir, 'renders_unit2', sha256, 'transforms.json')):
